...
As we can see from the below table, all models are performing better than the baseline model. In light of our data set's class imbalance, we expected the balanced models to perform better in cross validation. Comparing the scores of each model, we can see that the balanced are doing much better than their counterparts. We choose the logistic regression model and not the support vector machine model because both are doing equivalently well and we wanted to explore the hyperparameters of a logistic regression model.
